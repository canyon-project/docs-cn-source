...
一个基于 Monaco Editor 的前端代码覆盖率可视化组件（库）。提供行覆盖计数、变更行标记和语法块（语句/函数/分支）未覆盖高亮功能，适用于在网页中快速渲染单文件级别的覆盖率详情。
...
| 颜色 | pct (覆盖率百分比)     | 描述                                           |
| ---- | --------------------- | ---------------------------------------------- |
| 绿色 | pct >= 80             | 语句覆盖率大于等于 80%              |
| 黄色 | pct >= 50 && pct < 80 | 语句覆盖率大于等于 50% 且小于 80% |
| 红色 | pct < 50              | 语句覆盖率小于 50%                  |
...
```javascript
  视图模式切换：在"代码树"和"文件列表"视图之间切换
  变更文件筛选："仅显示变更"开关，筛选显示有代码变更的文件
  文件搜索：输入文件路径关键词进行搜索筛选
  文件统计：显示当前筛选条件下的文件总数
```
...
```javascript
  ...
  覆盖率统计概览：显示关键覆盖率指标
    ...
    行覆盖率 (Lines)
    新增行覆盖率 (New Lines)
    变更函数覆盖率 (Changed Functions)
    变更分支覆盖率 (Changed Branches)
    变更语句覆盖率 (Changed Statements)
```
...
```javascript
  绿色：在行号旁显示执行计数。
  深绿色：在行号旁显示变更标记，通过传递 diff 高亮显示变更行。
  红色：未覆盖的语句和函数
  黄色：未完全覆盖的分支代码
...
| onSelect               | `(val: string) => Promise<{fileCoverage: FileCoverageData;fileContent: string; fileCodeChange: number[]; }>` | 文件选择回调           |
| defaultOnlyShowChanged | `boolean`                                                                                                    | 默认是否仅显示变更文件 |
